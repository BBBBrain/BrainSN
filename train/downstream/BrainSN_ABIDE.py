...
class MyDataset(Dataset):
    def __init__(self, data_folder,flag="train"):
        ...
        # file_list.sort(key= lambda x:x[-8:-4])#.png所以是[:-4]
    ...
    def __getitem__(self, index):
        ...
        s = asd[desired_column.index(int(self.filenames[index].split("/")[-1][:-4][-5:]))]
    ...
    def preprocess(self, examples,tr):
        ...
        signal_vector = examples[:70,:].transpose()  
        T = TimeSeries(signal_vector, sampling_interval=2)
        F = FilterAnalyzer(T, ub=0.1, lb=0.01)  
...
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.fc(self.bn(hn)) # Output shape (b, output_size)
        return output
...
m_model = MyModel().to(device)

learning_rate = 0.00001  ## 0.0001
optimizer = torch.optim.Adam(m_model.parameters(), lr=learning_rate, weight_decay=1e-2)  ## 1e-2
...
for epoch in range(num_epochs):
    for i, (inputs, labels,name) in enumerate(train_loader):
        ...
        outputs_series_latent = outputs_series["logits"][0][:,:,:] 
        
        outputs = m_model(outputs_series_latent)
    ...
    with torch.no_grad():
        ...
        # sen = recall_score(np.array(lab), np.array(pred))
        ...
        print('ACC{:.4f}%'.format(accuracy),'F1{:.4f}%'.format(f1_score_value),'SEN{:.4f}%'.format(sen),'SPE{:.4f}%'.format(spe))
